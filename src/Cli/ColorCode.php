...
class ColorCode
{
     ...
     *
     * @param string $string
     *            e.g 'fg=white;bg=black;options=bold,underscore;extra=1'
     *            
     * @return static
     * @throws InvalidArgumentException
     ...
     * @param array $options
     *            Style options. e.g ['bold', 'underscore']
     * @param bool $extra
     *
     * @throws InvalidArgumentException
     */
    public function __construct($fg = '', $bg = '', array $options = [], bool $extra = false)
    {
        if ($fg) {
            if (! isset(self::KNOWN_COLORS[$fg])) {
                throw new InvalidArgumentException(sprintf('Invalid foreground color "%1$s" [%2$s]', $fg, implode(', ', self::getKnownColors())));
            }
        ...
        if ($bg) {
            if (! isset(self::KNOWN_COLORS[$bg])) {
                throw new InvalidArgumentException(sprintf('Invalid background color "%1$s" [%2$s]', $bg, implode(', ', self::getKnownColors())));
            }
        ...
        foreach ($options as $option) {
            if (! isset(self::KNOWN_OPTIONS[$option])) {
                throw new InvalidArgumentException(sprintf('Invalid option "%1$s" [%2$s]', $option, implode(', ', self::getKnownOptions())));
            }
